...
##############################################################################
def get_fname_digest_mapping(inpath, alg, savepath=None, processes=None):
    """Compute fname to digest mapping

    :param inpath: Path to the input
    :param alg: Computation mode. Compute hashes of FILES using the algorithms specified. Legal values are
    ...
    if os.path.exists(savepath):
        response = raw_input("Savepath already exists, do you want to skip the hashdeep step yes/no? [yes]")
        if not response.lower() == "no":
            do_hashing = False
    if do_hashing:
        inpath = os.path.abspath(inpath)
        hashcmd = ['hashdeep', '-c', alg]
        if os.path.isfile(inpath):
            hashcmd.extend([inpath])
        elif os.path.isdir(inpath):
            hashcmd.extend(['-r', inpath])
        else:
            pass
        if processes:
            hashcmd.extend(['-j1%d' % int(processes)])
        result = check_output(hashcmd)
        if savepath:
            open(savepath, 'w').write(result)

    else:
        result = open(savepath, 'r').read()

    # Step 2: process
    fname_digest_dict = {}
    for line in result.split('\n'):
        if line and line[0] not in "%#":
            filesize, digest, filepath = line.split(',')
            fname_digest_dict[filepath] = digest
    return fname_digest_dict
